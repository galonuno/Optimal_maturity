...
Rest_Issuances            = parameters.Rest_Issuances; %string with Yes or No. 
Issuance_NonZero_V        = precompute.Issuance_NonZero_V ; %vector of zeros and ones depending on which maturities are issued
...
for iter = 1:max_iter
    ...
    results         = optimal_path(parameters,precompute,fRSS);
    ...
    %% Optimal policies
     if Rest_Issuances == 'Y'
        ...
        iota_rss = Issuance_NonZero_V.*iota_rss;
     elseif Rest_Issuances == 'N'  
        iota_rss = 1/lambda_bar * (Vrss + psi_rss );
     else
        disp('Input Error, Res_Issuances should be Yes or No string');
     end 
...
%% Value Functional Calculation
% Even though the government does not issue all the maturities

Value_Functional = sum(Vrss.*fRSS) * dt * (c_rss)^(-gamma);

toc

results.Value_Functional = Value_Functional;  % Value Function
results.V_rss            = Vrss;    % Valuations
