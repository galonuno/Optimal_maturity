...
%% Model parameters
% Model is annual, this we need to keep it fixed
% Tolerance
...
parameters.relax      = .1;%0.0001 ;  % relaxation coefficient
parameters.relax_path = .1;%0.0001 ;  % relaxation coefficient optimal path

% Numerical parameters
parameters.t_max     = 200  ; % maximum number of years (time)
parameters.tau_max   = 20   ; % maximum number of years (maturity)
parameters.dt        = 1/12  ; % monthly steps

% Restricted Maturities

parameters.Rest_Issuances    = 'Y';                 %Insert 'Y' Yes or 'N' no if you want restricted issuances     
parameters.Issued_Maturities = [1/12 6/12 1 2 3 4 5 10 11 12 13 15 18 20]; %Insert the maturities that will be issued. 
                                                       %In years. You can plug in as many as you want. 

% Preference Parameters
parameters.gamma   = 4.00 ;    % risk aversion
parameters.rho     = 0.04 ;    % discount factor - Model is Quarterly
...
%Interest Rate Shock Parameters: Uniform shocks, any number
parameters.r_bar     = 0.02  ; % steady-state short rate
parameters.delta     = 0.02  ; % coupon portion

parameters.S           = 2;       %number of shocks to the interest rate
%parameters.r_bar_0     = 0.02;
parameters.r_bar_0_min = 0.02;   %equal probability for the shocks
parameters.r_bar_0_max = 0.05;
parameters.rho_r_bar   = 0.995;%0.995 ; % persistence (in months)

%Risky Steady State Parameters
parameters.phi  = 0.125;       % Poisson arrival rate

% adjustment cost
parameters.lambda_bar = 50;

%% Step 1: PRECOMPUTE Bond Prices in Risky Steady State and Issued Maturities
% psi(tau,t,r0) is a 3 dimensional array 
% psi_rss (tau) is a vector 
% tau = maturity, r0 =  initial rate after the shock
...
%r_bar_0      =  parameters.r_bar_0;
r_bar_0_min  =  parameters.r_bar_0_min;
r_bar_0_max  =  parameters.r_bar_0_max;
...
%Prices after the shock
r_bar_0_v = linspace (r_bar_0_min,r_bar_0_max,S); 
p_r_bar_0 = (1/S)*ones(S,1); 
psi       = zeros (I,N,S); 
r_bar_n   = zeros (N,S);

for s=1:S 
parameters.r_bar_0   = r_bar_0_v (s);
[psi(:,:,s),r_bar_n(:,s)]           = bond_prices(parameters);
end

precompute.psi=psi;

%Prices before the shock

psi_rss            = bond_prices_rss(parameters,precompute);
precompute.psi_rss = psi_rss;
parameters.psi_rss = precompute.psi_rss;

%Restricted Issuances Precompute

Issued_Maturities         = parameters.Issued_Maturities;
Index_1_year              = round (I/tau_max);
Issued_Indices            = round (Index_1_year*Issued_Maturities); %the index of the vector of issuances that will be non-zero
Issuance_NonZero_V        = zeros(I,1);

h=1; 
for i =  1:I
    if i  == Issued_Indices(h)
        Issuance_NonZero_V(i)=1;
        h=h+1;
end
end

precompute.Issuance_NonZero_V = Issuance_NonZero_V;
    
%% Step 2: Risky Steady State

results   = risky_SS(parameters,precompute);
...
disp('Certainity equivalent of Consumption = C_eq_cons')
C_eq_cons = (parameters.rho*results.Value_Functional*(1-parameters.gamma))^(1/(1-parameters.gamma))
...
%plot(t,r_bar_n*100,'Linewidth',line_width,'Color',color_base); hold on;
