...
parameters.penalty  = -inf;        % Autarky penalty
...
execute_codes
close all; plotting; cd figures; save PF_R; figure(2);   orient landscape; saveas(gcf,'DSS','pdf'); saveas(gcf,'DSS','fig'); figure(3);  saveas(gcf,'PF_IR','pdf'); saveas(gcf,'PF_IR','fig');close all; cd ..
parameters.r_bar_0(1:N_r,1)   = 0.04        ; % [s*] initial value 
% % 1.2 Output shock
%Interest Rate Shock Parameters
parameters.y_0(1:N_y,1)   = 0.95; % [s*] initial value, output after a shock
execute_codes
close all; plotting; cd figures; save PF_Y; figure(3); saveas(gcf,'PF_Y','pdf');saveas(gcf,'PF_Y','fig');close all;  cd ..
parameters.y_0(1:N_y,1)   = 1; % [s*] initial value, output after a shock
