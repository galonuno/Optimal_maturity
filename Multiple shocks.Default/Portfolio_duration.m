function [average_duration, average_term_to_maturity] = Portfolio_duration(results, parameters, paths)
% Duration
...
for i=1:N_t
    ...
    if i > N_t - N_tau - 1
        ...
        interest_rate = cumsum(r_bar_n(i:i+N_tau-1))*dt;
        discount  = exp(-interest_rate);
        PV(1,i)   = discount(1);
        w_PV(1,i) = discount(1)*tau(1);
        duration_bond(1,i) = w_PV(1,i)/PV(1,i);
        for j=2:N_tau
            PV(j,i)   = delta*sum(discount(1:j-1)*dt) + discount(j);
            w_PV(j,i) = delta*sum(discount(1:j-1).*tau(1:j-1)*dt) + discount(j)*tau(j);
